""" TASK 1;
A number is said to be Harshad if it's exactly divisible by the sum of its digits. Create a function that determines whether
...
# for i in pluralize:
#     count += 1
#     if i not in pluralize[count:]:
#         new.append((pluralize.count(i) > 1) and (i + "s") or i)
...
# for i in range(len(listem)):
#     num_split.append(int(listem[i]) * 10 ** (len(listem) - i - 1))
...
# name = input("Kuruslusun adını aralarında boşluk bırakarak giriniz: ")
# listem = name.title().split()

# bas_harfler = ""
# for i in range(len(listem)):
#     print(listem[i][0], end = ".")
...
# N = int(input("Günde kaç defa elinizi yıkarsınız : "))
# M = int(input("Kaç ay boyunca bunu yaptınız : "))
# D = M * 30
# W = D * N
...
TASK 7;
Question 2
Write a function that finds the largest even number in a list. Return -1 if not found. The use of built-in functions max()
and sorted() are prohibited.
Examples
...
is odd, and a number is Evenish if the sum of all of its digits is even. If a number is Oddish, return "Oddish". Otherwise, return
"Evenish".
...
For this challenge, slice each fruit in half and sort the chunks alphabetically. This recipe tastes best when the chunks
are joined together to make a string.
Worked Example
...
# vowel = set("aeiouAEIOU") # Sadece ingilizce alfabedeki vowels lar dikkate alındı!
# new_word = ""
# for i in word:
#     if i in vowel:
#         i = i*(num+1)
