...
CREATE VIEW ombined_table AS
SELECT   market_fact.Ord_id, market_fact.Prod_id, market_fact.Ship_id, market_fact.Cust_id, market_fact.Sales, market_fact.Discount, market_fact.Order_Quantity, 
...
SELECT * INTO combined_table FROM dbo.ombined_table
...
select top 3 Cust_id,COUNT(distinct Ship_id) count_of_orders 
from combined_table group by Cust_id order by COUNT(distinct Ship_id) desc
...
select ay MONTH, COUNT(Cust_id) MONTHLY_NUM_OF_CUST from alt 
where Cust_id IN (SELECT Cust_id FROM cuslis)
group by ay
...
select Cust_id,
               SUM(CASE
			   when Prod_id IN ('Prod_11') Then 1 Else 0 END) P11,
			   SUM(CASE
			   WHEN Prod_id IN ('Prod_14')  THEN 1 ELSE 0 END) P14, 
...
-- vb.
	...
	select Cust_id, COUNT( DISTINCT FORMAT(Order_Date,'yyyy-MM')) Sip_Ad
	from combined_table 
	group by Cust_id
	Order by  COUNT( DISTINCT FORMAT(Order_Date,'yyyy-MM')) desc
